...
class SAC:
    ...
    def select_action(self, state: np.ndarray, evaluate: bool = False) -> np.ndarray:
        ...
        In training mode, actions are sampled stochastically from the policy.
        In evaluation mode, deterministic actions (mean of the policy) are used.
        
        Args:
            state: Current state observation as numpy array.
            evaluate: Whether in evaluation mode. If True, returns deterministic 
                actions. If False, returns stochastic actions.
            
        Returns:
            Selected action as numpy array within the action range.
            
        Example:
            >>> agent = SAC(state_dim=4, action_dim=2)
